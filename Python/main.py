...
def getuser(userInfo):
    data = {"username": userInfo["username"], "password": str2md5(userInfo["password"])}
    return data
...
def signHandler(userInfo, sence):
    ...
    if sence == 2:
        if is_sign_in:
            ...
            pushMessge(
                userInfo["pushPlusToken"],
                f"已经进行过签到，本次操作将更新打卡时间...\n{log_msg}",
            )
            msg.append(
                f'账号 {userInfo["username"]} 已经进行过签到，本次操作将更新打卡时间...\n{log_msg}'
            )
        elif is_sign_out:
            log("已签退，无法进行签到!")
        ...
        if is_sign_in:
            if is_sign_out:
                log("已经进行过签退，本次操作未进行...")
                pushMessge(
                    userInfo["pushPlusToken"], "已经进行过签退，本次操作未进行..."
                )
                msg.append(
                    f'账号 {userInfo["username"]} 已经进行过签退，本次操作未进行...'
                )
            else:
                log_msg = newSign(sessionId, signFormData, sign_header(signFormData))
                log("签退完成！！")
...
def pushMessge(token, message):
    ...
    requestData = {"token": token, "title": "校友邦通知", "content": message}
